<?php
class <%= fullModuleName %>_Adminhtml_<%= name %>Controller
    extends Mage_Adminhtml_Controller_Action
{
    ...
    public function indexAction()
    {
        ...
        $formBlock = $this->getLayout()
            ->createBlock('<%= moduleIdentifier %>/adminhtml_<%= name%>');

        // Add the grid container as the only item on this page
        $this->loadLayout()
            ->_addContent($formBlock)
            ->renderLayout();
    }
     ...
     */
    public function <%= mode.toLowerCase() %>Action()
    {
       if ($data = $this->getRequest()->getPost()) {
        ...
        // Add the container as the only item on this page
        $this->loadLayout()
            ->_addContent($formBlock)
            ->renderLayout();
    }
        ...
        switch ($actionName) {
            ...
            default:
                ...
                $isAllowed = $adminSession
                    ->isAllowed('<%= moduleIdentifier %>/<%= name%>');
                break;
        }
